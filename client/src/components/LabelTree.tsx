...
export default function LabelTree(props: IProps) {
  ...
  function getIcon(
    ...
    if (item.children && item.children.length > 0) {
      return item.isExpanded ? (
        ...
        <ChevronRightIcon style={{ marginBottom: '5px' }} onClick={() => onExpand(item.id)} />
      )
    }
  ...
  function onDragEnd(source: TreeSourcePosition, destination?: TreeDestinationPosition) {
    if (!destination) {
      return
    }
  ...
  function renderItem({ item, onExpand, onCollapse, provided, snapshot }: RenderItemParams) {
    return (
      <div ref={provided.innerRef} {...provided.draggableProps}>
        <NavigationItemWrapper isDragging={snapshot.isDragging}>
          {getIcon(item, onExpand, onCollapse)}
          <TagColor color={item.data.color_hex} {...provided.dragHandleProps} />
